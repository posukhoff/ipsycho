...
- [ ] On a phone, exercise Today, Tasks, task details, Reminders and Settings **in the Mini App**; in chat verify a card's own buttons edit the current card when possible, a stale button answers with a toast and loses its keyboard, and payloads remain within Telegram's 64-byte limit.
- [ ] Tap a button on a card from before the cleanup (`/tasks`, a list row, «⚙️ Ещё», «📅 Другая дата», a settings toggle) and verify it answers with one sentence and a launch button rather than spinning; repeat with `WEBAPP_ENABLED=false` and verify the sentence arrives with no button. Verify the client's command menu lists only `/context`, `/cancel`, `/retry_ai`, `/status`, `/clear`, `/help` (plus `/invite` for the owner).
- [ ] As a new invited user: `/start` asks for the timezone first (buttons and a typed city such as «Берлин»), then digests, quiet hours and the weekly review, and ends with a line offering the app. Send a free-text message before consenting, tap «Согласен» and verify that message is processed without retyping.
- [ ] With Telegram set to English and no pinned language: Today, a task card, a reminder card, the applied report, a confirmation card and the morning digest contain no Cyrillic. Ask the bot to switch the interface language to Ukrainian in words and repeat.
...
- [ ] Tap Done and Skip on task cards and verify each shows «Вернуть как было» and that Undo restores the task; press «⏰ Через 15 мин» on a reminder and verify the task's own time did not move. Verify no card offers «Начал», «⚙️ Ещё» or «📅 Другая дата», and that cancelling a task and pausing a series are done in the app or by conversation.
...
- [ ] Pause a recurring series and verify the app's paused-series screen lists it; resume it there and verify the future dates come back, the row disappears and no Undo is offered for the resume (pausing again is the way back).
...
- [ ] Open the week screen in the app: verify the past week's line, the pool ordered with last week's untaken pick first, that a tap takes a task for the week and the same tap releases it, and that the eighth pick is refused with a toast.
- [ ] On the morning card, verify the day's plan, the «взято на неделю» block, and that its launch button opens Today in the app, where one tap sets a task for today.
- [ ] Verify the weekly card arrives on its configured day with the pool count and a launch button into the week screen, and that no review conversation starts anywhere.
...
- [ ] Let a dated task go overdue, then open the week screen and verify it is in the pool with «просрочено», leads the list, and that one tap takes it for the week and another gives it today.
...
- [ ] Open a task from the «⚠️ Просрочено» filter in the app, go back, and verify the same filter is still selected.
- [ ] Open the memory screen in the app and verify a sensitive fact is listed with 🔒. Send `/context` and verify the interview starts in chat, in the interface language, and that with `WEBAPP_ENABLED=true` one short line with a launch button into the profile screen follows it, and with the flag off nothing follows.
...
- [ ] Deploy migration `0028_week_pool.sql` with the application. Existing tasks have no week mark, so the pool starts empty of picks; verify the week screen lists the undated tasks and that picking one writes this week's Monday.
