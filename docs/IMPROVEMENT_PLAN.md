...
- **P2.9** — сделано полностью: четыре примера в промпте проверяются схемой контракта, живой прогон дал 20/20 в трёх прогонах (см. `docs/AGENT_FLOW.md` §9д).
...
**Что осталось (проверено на сервере 2026-09-05):**

1. **Бэкапы: сделан локальный стопгап (2026-09-05).** Ключ в `/home/deploy/ipsycho-secrets/backup.key`, cron 03:15 ежедневно (7 дневных + 4 недельных копии), первого числа — проверка восстановления в одноразовый контейнер, по воскресеньям — чистка образов. Проверено вживую: `backup_ok … offsite=no` и `restore_check_ok … tables=26`. **Осталось off-site:** копии лежат на той же машине, что и база, и ключ шифрования тоже — потеря диска означает потерю всего. Нужен bucket S3-совместимого хранилища и ключи к нему; после этого достаточно дописать `S3_BACKUP_URI` в строку cron. Ключ шифрования стоит держать ещё где-то, кроме сервера.

2. **Оценка расходов включена (2026-09-05).** `AI_PRICING_JSON` в проде: `gpt-5.4-mini` — 0.75 / 0.075 / 4.50 USD за миллион токенов (вход / кэшированный вход / выход), `gpt-4o-mini-transcribe` — 0.003 USD за минуту; цены с developers.openai.com/api/docs/pricing, помечены `revision: 2026-09-05`. Порог предупреждения `AI_MONTHLY_WARNING_USD=10`. Проверять цены при смене модели или тарифов — расчёт молча останется по старым.
3. **Dead-man switch: сделан (2026-09-05).** `HEALTHCHECK_PING_URL` в проде — maintenance пингует healthchecks.io после каждого успешного часового тика; плюс `scripts/watchdog.sh` в cron каждые 5 минут пишет владельцу в Telegram, если `/ready` молчит две проверки подряд, и когда приложение вернулось. `BACKUP_PING_URL` тоже настроен: второй чек с периодом в сутки пингуется после успешного бэкапа, так что молчание cron будет замечено.
4. **Ручные проверки `MANUAL_ACTIONS.md`** — требуют живого Telegram-аккаунта.

Eval прогнан на живом провайдере: 20 из 20 в трёх прогонах, базовая линия записана в `eval/baseline.json`.
