...
2. **Оценка расходов на AI выключена:** в проде нет `AI_PRICING_JSON`, поэтому `ai_usage.estimated_cost_usd` пуст и предупреждение о месячной трате (`AI_MONTHLY_WARNING_USD`) никогда не сработает.
