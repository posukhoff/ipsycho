...
3. **Dead-man switch: сделан (2026-09-05).** `HEALTHCHECK_PING_URL` в проде — maintenance пингует healthchecks.io после каждого успешного часового тика; плюс `scripts/watchdog.sh` в cron каждые 5 минут пишет владельцу в Telegram, если `/ready` молчит две проверки подряд, и когда приложение вернулось. Осталось `BACKUP_PING_URL` — нужен второй чек с периодом в сутки.
