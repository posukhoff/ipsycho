...
1. **Бэкапов нет вообще.** `/opt/ipsycho-secrets` отсутствует (ключ шифрования не создан), `crontab` у `deploy` пуст, `aws` не установлен, в `backups/` только дампы, которые делает деплой перед миграциями. То есть при потере диска терять нечего, кроме них. Нужны: bucket S3-совместимого хранилища и ключи к нему — без них `backup-compose.sh` не запускается по построению. Дальше: `openssl rand` в `/opt/ipsycho-secrets/backup.key`, cron 03:15 и ежемесячный restore-drill (команды — в `docs/DEPLOYMENT.md`).
