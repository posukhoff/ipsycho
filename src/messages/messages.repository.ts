...
export class MessagesRepository {
  ...
  async setStatus(workspaceId: string, messageId: string, status: MessageProcessingStatus): Promise<void> {
    await this.database.db.update(messages).set({
      status,
  ...
  async deferAiUntil(workspaceId: string, userId: string, messageId: string, until: Date): Promise<void> {
    await this.database.db.update(messages).set({ status: "waiting_ai", aiNextRetryAt: until }).where(and(
      eq(messages.workspaceId, workspaceId), eq(messages.userId, userId), eq(messages.id, messageId),
    ));
  }
  ...
  async scheduleAiRetry(workspaceId: string, userId: string, messageId: string, now = new Date()): Promise<{ retryCount: number; nextRetryAt: Date | null }> {
    const [current] = await this.database.db.select({ retryCount: messages.aiRetryCount }).from(messages)
      .where(and(eq(messages.workspaceId, workspaceId), eq(messages.userId, userId), eq(messages.id, messageId))).limit(1);
    const retryCount = (current?.retryCount ?? 0) + 1;
    const nextRetryAt = nextAutomaticAiRetryAt(retryCount, now);
    await this.database.db.update(messages).set({
      status: "waiting_ai", aiRetryCount: retryCount, aiNextRetryAt: nextRetryAt, aiLastErrorAt: now,
  ...
  async findDueAiRetries(now = new Date(), limit = 25) {
    return this.database.db.select({ message: messages, user: users, settings: userSettings })
      .from(messages)
      .innerJoin(users, eq(users.id, messages.userId))
      .innerJoin(userSettings, eq(userSettings.userId, messages.userId))
      .where(and(
        eq(messages.role, "user"), eq(messages.status, "waiting_ai"), lte(messages.aiNextRetryAt, now),
        eq(users.status, "active"), eq(users.aiStatus, "enabled"),
      ))
  ...
  async isAiProcessingAllowed(userId: string): Promise<boolean> {
    const [row] = await this.database.db.select({ id: users.id }).from(users).where(and(
      eq(users.id, userId), eq(users.status, "active"), eq(users.aiStatus, "enabled"),
    )).limit(1);
    return Boolean(row);
  }

  async countUserMessagesSince(userId: string, since: Date): Promise<number> {
    const [row] = await this.database.db.select({ count: sql<number>`count(*)::int` }).from(messages)
  ...
  async findLatestRetryable(workspaceId: string, userId: string) {
    const [row] = await this.database.db.select().from(messages).where(and(
      eq(messages.workspaceId, workspaceId),
      eq(messages.userId, userId),
      eq(messages.role, "user"),
      inArray(messages.status, ["waiting_ai", "blocked_consent"]),
    )).orderBy(desc(messages.createdAt)).limit(1);
    return row ?? null;
  }

  async claimRetryable(workspaceId: string, userId: string, messageId: string) {
    const [row] = await this.database.db.update(messages).set({ status: "processing", aiNextRetryAt: null }).where(and(
      eq(messages.workspaceId, workspaceId),
      eq(messages.userId, userId),
  ...
  async deleteRawOlderThan(cutoff: Date): Promise<number> {
    const rows = await this.database.db.delete(messages).where(and(
      lt(messages.createdAt, cutoff),
      inArray(messages.status, ["processed", "waiting_ai", "blocked_consent", "processing"]),
  ...
  async clearConversation(workspaceId: string, userId: string): Promise<number> {
    const deleted = await this.database.db.delete(messages).where(and(
      eq(messages.workspaceId, workspaceId), eq(messages.userId, userId),
    )).returning({ id: messages.id });
