...
import {
  ...
  taskGoals, taskOccurrences, taskRecurrenceExclusions, tasks, userSettings,
} from "../database/schema.js";
import { insertTaskPlan, type PersistedTaskPlan } from "../tasks/tasks.repository.js";
import {
  cancelTaskInTx, changeReminderInTx, changeSeriesInTx, completeOccurrenceInTx, completeTaskInTx, concretiseTaskInTx,
  occurrenceInteractionInTx, parseJsonDate, replaceChecklist, rescheduleOccurrenceInTx, updateOccurrenceInTx, updateSettingsInTx, updateTaskInTx,
  type CancelTaskStepResult, type ChangeReminderStepResult, type ChangeSeriesStepResult, type CompleteTaskStepResult, type ConcretiseTaskStepResult,
  type DbTransaction, type OccurrenceInteractionStepResult, type OccurrenceMutableState, type RescheduleOccurrenceStepResult,
  type SettingsMutableState, type SettingsOperation, type SettingsPatch, type TaskMutableState, type TouchedVersion,
  type UpdateOccurrenceStepResult, type UpdateSettingsStepResult, type UpdateTaskPatch, type UpdateTaskStepResult,
} from "./action-mutations.repository.js";
import {
  createGoalInTx, deleteMemoryInTx, goalPlanInTx, linkTaskToGoalInTx, saveMemoryInTx, unlinkTaskToGoalInTx, updateGoalInTx, updateMemoryInTx,
...
export type ActionGroupStep =
  ...
  | {
    kind: "update_goal"; goalId: string; expectedVersion: number;
...
export interface CreateTaskStepResult { kind: "create_task"; taskId: string; title: string; goalTitle: string | null }
...
interface UndoEvent { actionType: string; entityType: string; entityId: string; postVersion: number | null; beforeState: unknown; afterState: unknown }
...
export class ActionGroupRepository {
  ...
  async apply(input: {
    workspaceId: string; actorUserId: string; groupId: string; groupExists: boolean; sourceMessageId?: string;
    steps: readonly ActionGroupStep[]; now: Date; undoExpiresAt: Date;
    ...
    return this.database.db.transaction(async (tx) => {
      if (!input.groupExists) {
        await tx.insert(actionGroups).values({
          ...
          status: "applying", requiresConfirmation: false, createdAt: input.now,
        });
      }
      ...
      const result: ActionGroupApplyResult = {
        groupId: input.groupId, steps: [], reminderRebuildOccurrenceIds: [], fuzzyRebuildTaskIds: [], reconcileTaskIds: [], createdTaskIds: [], preparedPlans: [],
      };
        ...
        switch (step.kind) {
          case "create_task": {
            ...
            await tx.insert(actionEvents).values({
              ...
              entityId: step.plan.task.id, postVersion: 1, afterState: { title: step.plan.task.title },
            });
            let goalTitle: string | null = null;
            if (step.goalLink) {
              const link = await linkTaskToGoalInTx(tx, {
                ...scope, taskId: step.plan.task.id, expectedTaskVersion: 1,
                goalId: step.goalLink.goalId, expectedGoalVersion: versions.expect("goal", step.goalLink.goalId, step.goalLink.goalVersion),
                source: step.goalLink.source, confidence: step.goalLink.confidence,
              });
          ...
          case "update_task": {
            const { touched, ...stepResult } = await updateTaskInTx(tx, { ...scope, taskId: step.taskId, expectedVersion: versions.expect("task", step.taskId, step.expectedVersion), patch: step.patch });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "update_occurrence": {
            const { touched, ...stepResult } = await updateOccurrenceInTx(tx, { ...scope, occurrenceId: step.occurrenceId, expectedVersion: versions.expect("occurrence", step.occurrenceId, step.expectedVersion), operation: step.operation });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "occurrence_interaction": {
            const { touched, ...stepResult } = await occurrenceInteractionInTx(tx, {
              ...
              operation: step.operation, ...(step.details !== undefined ? { details: step.details } : {}),
            });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "complete_task": {
            const { touched, ...stepResult } = await completeTaskInTx(tx, { ...scope, taskId: step.taskId, expectedVersion: versions.expect("task", step.taskId, step.expectedVersion) });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "complete_occurrence": {
            const { touched, ...stepResult } = await completeOccurrenceInTx(tx, { ...scope, occurrenceId: step.occurrenceId, expectedVersion: versions.expect("occurrence", step.occurrenceId, step.expectedVersion) });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "cancel_task": {
            const { touched, ...stepResult } = await cancelTaskInTx(tx, { ...scope, taskId: step.taskId, expectedVersion: versions.expect("task", step.taskId, step.expectedVersion) });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          ...
          case "concretise_task": {
            const { touched, ...stepResult } = await concretiseTaskInTx(tx, {
              ...scope, taskId: step.taskId, expectedVersion: versions.expect("task", step.taskId, step.expectedVersion),
              definition: step.definition, occurrenceStatus: step.occurrenceStatus,
          ...
          case "change_reminder": {
            const { touched, ...stepResult } = await changeReminderInTx(tx, {
              ...scope, occurrenceId: step.occurrenceId, expectedVersion: versions.expect("occurrence", step.occurrenceId, step.expectedVersion),
              mode: step.mode, ...(step.rule ? { rule: step.rule } : {}),
            });
            versions.bump(touched); result.steps.push(stepResult);
          ...
          case "change_series": {
            const { touched, ...stepResult } = await changeSeriesInTx(tx, {
              ...scope, taskId: step.taskId, expectedVersion: versions.expect("task", step.taskId, step.expectedVersion),
              operation: step.operation, ...(step.editDefinition ? { editDefinition: step.editDefinition } : {}),
            });
            versions.bump(touched); result.steps.push(stepResult);
          ...
          case "create_goal": {
            const { touched, ...stepResult } = await createGoalInTx(tx, {
              ...scope, title: step.title, ...(step.why !== undefined ? { why: step.why } : {}), ...(step.targetLocalDate !== undefined ? { targetLocalDate: step.targetLocalDate } : {}),
            });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "update_goal": {
            const { touched, ...stepResult } = await updateGoalInTx(tx, { ...scope, goalId: step.goalId, expectedVersion: versions.expect("goal", step.goalId, step.expectedVersion), patch: step.patch });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "link_task_to_goal": {
            const { touched, ...stepResult } = await linkTaskToGoalInTx(tx, {
              ...scope, taskId: step.taskId, expectedTaskVersion: versions.expect("task", step.taskId, step.expectedTaskVersion),
              goalId: step.goalId, expectedGoalVersion: versions.expect("goal", step.goalId, step.expectedGoalVersion), source: step.source, confidence: step.confidence,
            });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "unlink_task_to_goal": {
            const { touched, ...stepResult } = await unlinkTaskToGoalInTx(tx, {
              ...scope, taskId: step.taskId, expectedTaskVersion: versions.expect("task", step.taskId, step.expectedTaskVersion),
              goalId: step.goalId, expectedGoalVersion: versions.expect("goal", step.goalId, step.expectedGoalVersion),
            });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          ...
          case "save_memory": {
            const { touched, ...stepResult } = await saveMemoryInTx(tx, {
              ...scope, memoryType: step.memoryType, content: step.content, sensitive: step.sensitive, source: step.source,
              ...(input.sourceMessageId ? { sourceMessageId: input.sourceMessageId } : {}),
            });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          ...
          case "update_memory": {
            const { touched, ...stepResult } = await updateMemoryInTx(tx, { ...scope, memoryId: step.memoryId, expectedVersion: versions.expect("memory", step.memoryId, step.expectedVersion), patch: step.patch });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "delete_memory": {
            const { touched, ...stepResult } = await deleteMemoryInTx(tx, { ...scope, memoryId: step.memoryId, expectedVersion: versions.expect("memory", step.memoryId, step.expectedVersion) });
            versions.bump(touched); result.steps.push(stepResult);
            break;
          }
          case "update_settings": {
            const { touched, ...stepResult } = await updateSettingsInTx(tx, {
              ...scope, expectedVersion: versions.expect("settings", input.actorUserId, step.expectedVersion), patch: step.patch,
  ...
  async undo(input: { workspaceId: string; groupId: string; now: Date }): Promise<ActionGroupUndoResult> {
    return this.database.db.transaction(async (tx) => {
      const events: UndoEvent[] = await tx.select().from(actionEvents).where(and(eq(actionEvents.workspaceId, input.workspaceId), eq(actionEvents.groupId, input.groupId)));
      const rebuild = new Set<string>();
      ...
      const lockAndVerify = async (entity: TouchedVersion["entity"], grouped: Map<string, UndoEvent[]>, loader: (ids: string[]) => Promise<Array<{ id: string; version: number }>>) => {
      ...
      await lockAndVerify("occurrence", occurrenceEvents, (ids) => tx.select({ id: taskOccurrences.id, version: taskOccurrences.version }).from(taskOccurrences).where(and(eq(taskOccurrences.workspaceId, input.workspaceId), inArray(taskOccurrences.id, ids))).orderBy(taskOccurrences.id).for("update"));
      await lockAndVerify("goal", goalEvents, (ids) => tx.select({ id: goals.id, version: goals.version }).from(goals).where(and(eq(goals.workspaceId, input.workspaceId), inArray(goals.id, ids))).orderBy(goals.id).for("update"));
      const restorableMemory = new Map([...memoryEvents].filter(([, list]) => !list.every((event) => event.actionType === "delete_memory")));
      await lockAndVerify("memory", restorableMemory, (ids) => tx.select({ id: memoryItems.id, version: memoryItems.version }).from(memoryItems).where(and(eq(memoryItems.workspaceId, input.workspaceId), inArray(memoryItems.id, ids))).orderBy(memoryItems.id).for("update"));
      await lockAndVerify("settings", settingsEvents, (ids) => tx.select({ id: userSettings.userId, version: userSettings.version }).from(userSettings).where(inArray(userSettings.userId, ids)).orderBy(userSettings.userId).for("update"));
      ...
      for (const [occurrenceId, list] of occurrenceEvents) {
        ...
        if (actionTypes.has("change_reminder")) {
          ...
          await tx.update(reminderDeliveries).set({ status: "cancelled", suppressedReason: "superseded" }).where(and(eq(reminderDeliveries.workspaceId, input.workspaceId), eq(reminderDeliveries.occurrenceId, occurrenceId), inArray(reminderDeliveries.status, ["pending", "processing"])));
        }
        ...
        const [restored] = await tx.update(taskOccurrences).set({
          ...
          skipReason: state.skipReason, needsReminderRebuild: needsRebuild ? true : state.needsReminderRebuild,
        ...
        if (actionTypes.has("update_occurrence") && followUpIds.length) {
          ...
          await tx.update(reminderDeliveries).set({ status: "pending", suppressedReason: null }).where(and(
            eq(reminderDeliveries.workspaceId, input.workspaceId), eq(reminderDeliveries.occurrenceId, occurrenceId),
            inArray(reminderDeliveries.reminderRuleId, followUpIds), eq(reminderDeliveries.status, "cancelled"), sql`${reminderDeliveries.scheduledFor} > ${input.now}`,
          ));
        }
        ...
        if (actionTypes.has("complete_occurrence") || actionTypes.has("update_occurrence")) {
          await tx.update(reminderDeliveries).set({ status: "pending", suppressedReason: null }).where(and(
            eq(reminderDeliveries.workspaceId, input.workspaceId), eq(reminderDeliveries.occurrenceId, occurrenceId),
            eq(reminderDeliveries.status, "suppressed"), eq(reminderDeliveries.suppressedReason, "no_longer_applicable"), sql`${reminderDeliveries.scheduledFor} > ${input.now}`,
          ));
        }
      }
      ...
      for (const [taskId, list] of taskEvents) {
        ...
        if (actionTypes.has("change_series")) {
          ...
          if (newerRevision.length) {
            ...
            await tx.update(reminderDeliveries).set({ status: "cancelled", suppressedReason: "superseded" })
              .where(and(eq(reminderDeliveries.workspaceId, input.workspaceId), inArray(reminderDeliveries.occurrenceId, ids), inArray(reminderDeliveries.status, ["pending", "processing"])));
          }
        }
        const current = versions.current("task", taskId);
        const [restored] = await tx.update(tasks).set({
          title: state.title, why: state.why, nextAction: state.nextAction, context: state.context, importance: state.importance, status: state.status,
          timeMode: state.timeMode, timezone: state.timezone, plannedStartAt: parseJsonDate(state.plannedStartAt), plannedEndAt: parseJsonDate(state.plannedEndAt),
          plannedLocalDate: state.plannedLocalDate, dueAt: parseJsonDate(state.dueAt), dueLocalDate: state.dueLocalDate,
        ...
        if (planningReviewRuleIds.length) {
          await tx.update(reminderRules).set({ active: true }).where(and(eq(reminderRules.workspaceId, input.workspaceId), inArray(reminderRules.id, planningReviewRuleIds)));
          await tx.update(reminderDeliveries).set({ status: "pending", suppressedReason: null }).where(and(
            eq(reminderDeliveries.workspaceId, input.workspaceId), inArray(reminderDeliveries.reminderRuleId, planningReviewRuleIds),
            eq(reminderDeliveries.status, "suppressed"), eq(reminderDeliveries.suppressedReason, "no_longer_applicable"), sql`${reminderDeliveries.scheduledFor} > ${input.now}`,
          ));
        }
      ...
      for (const [memoryId, list] of memoryEvents) {
        if (list.every((event) => event.actionType === "delete_memory")) {
          ...
          await tx.insert(memoryItems).values({
            ...
            version: state.version + 1, createdAt: new Date(state.createdAt), updatedAt: input.now,
          });
          continue;
        }
        if (list.some((event) => CREATE_ACTIONS.has(event.actionType))) {
      ...
      for (const [userId, list] of settingsEvents) {
        ...
        const [restored] = await tx.update(userSettings).set({
          ...
          weekdayQuietStart: state.weekdayQuietStart, weekdayQuietEnd: state.weekdayQuietEnd, weekendQuietStart: state.weekendQuietStart, weekendQuietEnd: state.weekendQuietEnd,
...
async function lockTargets(tx: DbTransaction, workspaceId: string, actorUserId: string, steps: readonly ActionGroupStep[]): Promise<VersionTracker> {
  ...
  const createdTaskIds = new Set(steps.flatMap((step) =>
    step.kind === "create_task" ? [step.plan.task.id] : step.kind === "goal_plan" ? step.plans.map((plan) => plan.task.id) : [],
  ));
  for (const step of steps) {
    switch (step.kind) {
      case "create_task": if (step.goalLink) goalIds.add(step.goalLink.goalId); break;
      case "update_task": case "complete_task": case "cancel_task": case "concretise_task": case "change_series": taskIds.add(step.taskId); break;
      case "update_occurrence": case "occurrence_interaction": case "complete_occurrence": case "reschedule_occurrence": case "change_reminder": occurrenceIds.add(step.occurrenceId); break;
      case "update_goal": goalIds.add(step.goalId); break;
      case "link_task_to_goal": case "unlink_task_to_goal": taskIds.add(step.taskId); goalIds.add(step.goalId); break;
      case "update_memory": case "delete_memory": memoryIds.add(step.memoryId); break;
      case "update_settings": settings = true; break;
      default: break;
    }
  }
