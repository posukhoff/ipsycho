...
const C = {
  ru: {
    create: "Создать", plan: (goal: string, n: number) => `Создать цель «${goal}» и ${n} ${plural("ru", n, "задачу", "задачи", "задач")}`, updateTask: "Изменить задачу", titleTo: "название →", importanceTo: "важность →",
    importance: { critical: "критическая", required: "обязательная", normal: "обычная" }, habitOn: "включить режим привычки", habitOff: "выключить режим привычки", checklist: "чеклист", why: "зачем", nextAction: "следующий шаг", context: "контекст",
    series: " (всю серию)", done: "Отметить выполненной", start: "Начать", skip: "Пропустить", cancel: "Отменить", blocker: "Записать блокер", seen: "Отметить увиденной",
    rescheduleSeries: "Изменить расписание серии", reschedule: "Перенести", clearReminders: "Убрать напоминания", addReminder: "Добавить напоминание", replaceReminder: "Заменить напоминание",
    createGoal: "Создать цель", link: "Связать", withGoal: "с целью", unlink: "Отвязать", fromGoal: "от цели", goalTo: "Цель →", goalStatus: { completed: "завершена", paused: "на паузе", cancelled: "отменена", active: "активна" }, updateGoal: "Изменить цель",
    deleteMemory: "Удалить запись из памяти", updateMemory: "Изменить запись в памяти", remember: "Запомнить", sensitive: " (чувствительное)", settings: "Изменить настройки",
    by: "до", min: "мин", bypass: " — игнорируя тихие часы", ofDue: "срока", ofEnd: "конца", ofStart: "начала", before: (n: number) => `за ${n} мин до`, after: (n: number) => `через ${n} мин после`, at: "в момент", atTime: "в", days: "дн",
    ops: { timezone: "часовой пояс →", language: "язык", digest: "сводка", weekly_review: "недельный обзор", quiet_hours: "тихие часы", snooze: "пауза уведомлений", reminder_defaults: "напоминания по умолчанию" },
  },
  uk: {
    create: "Створити", plan: (goal: string, n: number) => `Створити ціль «${goal}» і ${n} ${plural("uk", n, "завдання", "завдання", "завдань")}`, updateTask: "Змінити завдання", titleTo: "назва →", importanceTo: "важливість →",
    importance: { critical: "критична", required: "обов'язкова", normal: "звичайна" }, habitOn: "увімкнути режим звички", habitOff: "вимкнути режим звички", checklist: "чекліст", why: "навіщо", nextAction: "наступний крок", context: "контекст",
    series: " (усю серію)", done: "Позначити виконаним", start: "Почати", skip: "Пропустити", cancel: "Скасувати", blocker: "Записати блокер", seen: "Позначити побаченим",
    rescheduleSeries: "Змінити розклад серії", reschedule: "Перенести", clearReminders: "Прибрати нагадування", addReminder: "Додати нагадування", replaceReminder: "Замінити нагадування",
    createGoal: "Створити ціль", link: "Пов'язати", withGoal: "з ціллю", unlink: "Відв'язати", fromGoal: "від цілі", goalTo: "Ціль →", goalStatus: { completed: "завершена", paused: "на паузі", cancelled: "скасована", active: "активна" }, updateGoal: "Змінити ціль",
    deleteMemory: "Видалити запис із пам'яті", updateMemory: "Змінити запис у пам'яті", remember: "Запам'ятати", sensitive: " (чутливе)", settings: "Змінити налаштування",
    by: "до", min: "хв", bypass: " — ігноруючи тихі години", ofDue: "терміну", ofEnd: "кінця", ofStart: "початку", before: (n: number) => `за ${n} хв до`, after: (n: number) => `через ${n} хв після`, at: "у момент", atTime: "о", days: "дн",
    ops: { timezone: "часовий пояс →", language: "мова", digest: "зведення", weekly_review: "тижневий огляд", quiet_hours: "тихі години", snooze: "пауза сповіщень", reminder_defaults: "нагадування за замовчуванням" },
  },
  en: {
    create: "Create", plan: (goal: string, n: number) => `Create goal “${goal}” with ${n} ${n === 1 ? "task" : "tasks"}`, updateTask: "Edit task", titleTo: "title →", importanceTo: "importance →",
    importance: { critical: "critical", required: "required", normal: "normal" }, habitOn: "enable habit mode", habitOff: "disable habit mode", checklist: "checklist", why: "why", nextAction: "next step", context: "context",
    series: " (whole series)", done: "Mark done", start: "Start", skip: "Skip", cancel: "Cancel", blocker: "Record blocker", seen: "Mark seen",
    rescheduleSeries: "Change the series schedule", reschedule: "Move", clearReminders: "Remove reminders", addReminder: "Add reminder", replaceReminder: "Replace reminder",
    createGoal: "Create goal", link: "Link", withGoal: "to goal", unlink: "Unlink", fromGoal: "from goal", goalTo: "Goal →", goalStatus: { completed: "completed", paused: "paused", cancelled: "cancelled", active: "active" }, updateGoal: "Edit goal",
    deleteMemory: "Delete a memory note", updateMemory: "Edit a memory note", remember: "Remember", sensitive: " (sensitive)", settings: "Change settings",
    by: "by", min: "min", bypass: " — ignoring quiet hours", ofDue: "the deadline", ofEnd: "the end", ofStart: "the start", before: (n: number) => `${n} min before`, after: (n: number) => `${n} min after`, at: "at", atTime: "at", days: "d",
    ops: { timezone: "timezone →", language: "language", digest: "briefing", weekly_review: "weekly review", quiet_hours: "quiet hours", snooze: "notification pause", reminder_defaults: "default reminders" },
  },
} as const;
...
export function describeAction(action: ResolvedAction, locale: Locale = "ru", names: ActionNames = {}): string {
  ...
  const taskName = (id: string) => { const title = names.tasks?.get(id); return title ? ` ${q(title)}` : ""; };
  const goalName = (id: string | null) => { const title = id ? names.goals?.get(id) : undefined; return title ? ` ${q(title)}` : ""; };
  switch (action.type) {
    case "create_task":
    ...
    case "memory":
      if (action.op === "delete") return `${c.deleteMemory}${action.memoryId && names.memory?.get(action.memoryId) ? `: ${q(names.memory.get(action.memoryId)!.slice(0, 80))}` : ""}`;
...
export function describeWhen(when: When, locale: Locale = "ru"): string {
  ...
  switch (when.mode) {
    ...
    case "deadline": return ` — ${c.by} ${date(when.date)}${when.time ? ` ${when.time}` : ""}`;
    case "date": return ` — ${date(when.date)}`;
    case "exact": return ` — ${date(when.date)} ${when.time}${when.durationMinutes ? ` (${when.durationMinutes} ${c.min})` : ""}`;
  }
}
...
function settingsOperationLabel(action: ResolvedActionOf<"settings">, locale: Locale): string {
  ...
  switch (action.operation) {
    case "timezone": return `${c.ops.timezone} ${action.timezone ?? ""}`;
    case "language": return `${c.ops.language}${action.language ? ` → ${action.language}` : ""}`;
    case "digest":
    ...
    case "reminder_defaults": return c.ops.reminder_defaults;
  }
}
