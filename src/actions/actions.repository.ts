...
export class ActionsRepository {
  ...
  async createImmediateGroup(input: { id: string; workspaceId: string; actorUserId: string; sourceMessageId?: string }): Promise<void> {
  ...
  async findPendingGroup(
    ...
    const [group] = await this.database.db
      .select({ id: actionGroups.id, createdAt: actionGroups.createdAt })
      .from(actionGroups)
      .where(and(eq(actionGroups.workspaceId, workspaceId), eq(actionGroups.id, groupId), eq(actionGroups.actorUserId, actorUserId), eq(actionGroups.status, "pending")))
      .limit(1);
    if (!group) return null;
    const rows = await this.database.db
      .select({ actionType: pendingActions.actionType, payload: pendingActions.payload, expiresAt: pendingActions.expiresAt })
      .from(pendingActions)
  ...
  async claimPendingGroup(workspaceId: string, actorUserId: string, groupId: string, now: Date) {
    ...
    const actions = await this.database.db
      .select()
      .from(pendingActions)
      .where(and(eq(pendingActions.workspaceId, workspaceId), eq(pendingActions.groupId, groupId)));
    if (!actions.length || actions.some((action) => action.expiresAt <= now)) {
      await this.database.db.transaction(async (tx) => {
        await tx
          .update(actionGroups)
          .set({ status: "expired" })
          .where(and(eq(actionGroups.workspaceId, workspaceId), eq(actionGroups.id, groupId)));
  ...
  async expireLegacyPendingGroups(now: Date, isValid: (actionType: string, payload: unknown) => boolean): Promise<number> {
    return this.database.db.transaction(async (tx) => {
      const rows = await tx
        .select({ workspaceId: pendingActions.workspaceId, groupId: pendingActions.groupId, actionType: pendingActions.actionType, payload: pendingActions.payload })
        .from(pendingActions);
      ...
      for (const row of legacy.values()) {
        const [cancelled] = await tx
          .update(actionGroups)
          .set({ status: "cancelled" })
          .where(and(eq(actionGroups.workspaceId, row.workspaceId), eq(actionGroups.id, row.groupId), eq(actionGroups.status, "pending")))
          .returning({ id: actionGroups.id });
        ...
        await tx.insert(actionEvents).values({
          ...
          groupId: row.groupId,
          actionType: "legacy_contract_expired",
          entityType: "action_group",
  ...
  async expirePendingGroups(now = new Date()): Promise<number> {
    const expired = await this.database.db.select({ id: pendingActions.id, groupId: pendingActions.groupId }).from(pendingActions).where(lte(pendingActions.expiresAt, now));
    ...
    await this.database.db.transaction(async (tx) => {
      await tx
        .update(actionGroups)
        .set({ status: "expired" })
        .where(and(inArray(actionGroups.id, groupIds), eq(actionGroups.status, "pending")));
