...
export const ru = {
  ...
  // Account
  delete_prompt: "Удаление остановит AI, сводки и напоминания сразу. Данные будут окончательно удалены через {days} дней; до этого аккаунт можно восстановить командой /restore. Остаточные данные могут сохраняться в зашифрованных резервных копиях: {daily} ежедневных и {weekly} еженедельных.",
  ...
  // Consent
  consent_prompt: "Для AI-чата текст сообщений будет отправляться внешнему провайдеру {provider}. Голосовые при OpenAI отправляются только для расшифровки и не сохраняются как аудио. Разрешить такую обработку?",
  ...
  consent_declined: "Хорошо, AI выключен. Кнопки, напоминания и сводки продолжат работать; свободный текст я разбирать не буду. Включить можно в любой момент: просто напиши мне ещё раз.",
  voice_consent_prompt: "Для AI-обработки текст будет отправляться внешнему провайдеру, а голосовое — OpenAI только для расшифровки. Аудио не сохраняется; распознанный текст обрабатывается и хранится как обычное сообщение. Разрешить?",
