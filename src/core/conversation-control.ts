...
export function detectConversationControl(text: string): ConversationControl {
  ...
  if (/(?:хватит\s+(?:вопросов|спрашивать)|не\s+задавай\s+(?:больше\s+)?вопрос(?:ов|ы)?|сделай\s+вывод|дай\s+вывод|подведи\s+итог|досить\s+(?:питань|запитань)|зроби\s+висновок|підведи\s+підсумок)/u.test(value)) {
    return "conclude";
  }
