...
export function reviewCorrection(kind: ReviewKind, forceConclusion = false): string {
  if (kind === "weekly") {
    if (forceConclusion) {
      return "This is the final turn of a weekly planning session after the allowed focused questions. Give a concise plan from the known context, return question=null, and do not ask anything else. Return actions=[]: nothing is created or moved in this final summary.";
    }
