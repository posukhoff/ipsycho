...
const PLANNING_CHORE =
  /(?:поставить|установить|настроить|добавить|создать|завести|записать)\s+(?:себе\s+)?(?:напоминани|задач|событи|в\s+календар|в\s+список)|напомнить\s+себе|запланировать\s+(?:задач|это|её|его|время|дату)|^запланировать(?!\p{L})|открыть\s+(?:это\s+|этот\s+)?(?:приложени|бот|список|календар)|выбрать\s+(?:время|дату)|решить,?\s+когда|(?:поставити|встановити|налаштувати|додати|створити|записати)\s+(?:собі\s+)?(?:до\s+|у\s+|в\s+)?(?:нагадуванн|завданн|поді|календар|списку?\s+завдань|список)|нагадати\s+собі|запланувати\s+(?:завданн|це|її|його|час|дату)|^запланувати(?!\p{L})|відкрити\s+(?:цей\s+|цю\s+)?(?:застосунок|бот|список|календар)|обрати\s+(?:час|дату)|вирішити,?\s+коли|set\s+(?:a\s+|the\s+)?reminder|put\s+(?:it|this)\s+(?:in|on)\s+(?:the\s+)?(?:calendar|list)|add\s+(?:a\s+|it\s+|this\s+)?(?:to\s+)?(?:the\s+)?(?:task|todo|to-do|reminder|calendar|list)|schedule\s+(?:it|this|the\s+task|a\s+time)|(?:pick|choose)\s+(?:a\s+)?(?:time|date)|decide\s+when|open\s+the\s+app/iu;
...
const SCHEDULING_ECHO =
  /^(?:чтобы\s+)?(?:не\s+(?:забыть|пропустить|упустить|опоздать)(?!\p{L})|напомнить\s+(?:себе|мне|о|об|про)(?!\p{L})|вспомнить(?!\p{L}))|^(?:это\s+|есть\s+)?(?:(?:контрольное|отдельное|повторное|новое|дополнительное)\s+)*напоминани[еяюи](?!\p{L})|^запланирован[оаы]?(?!\p{L})|^(?:щоб\s+)?(?:не\s+(?:забути|пропустити|запізнитися)(?!\p{L})|нагадати\s+(?:собі|мені|про)(?!\p{L})|згадати(?!\p{L}))|^(?:це\s+|є\s+)?(?:(?:контрольне|окреме|повторне|нове|додаткове)\s+)*нагадуванн[яю](?!\p{L})|^заплановано(?!\p{L})|^(?:so\s+(?:i|we)\s+)?(?:don'?t\s+forget|do\s+not\s+forget)(?!\p{L})|^(?:to\s+)?(?:remember|remind\s+me|not\s+to\s+forget)(?!\p{L})|^(?:a\s+|the\s+|this\s+)?(?:follow-?up\s+|extra\s+|repeat\s+|separate\s+)*reminder(?!\p{L})|^scheduled\s+(?:for|on|at)(?!\p{L})/iu;
