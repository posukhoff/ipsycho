...
export class ChatService {
  ...
  get maxMessagesPerHour(): number {
    return this.ai.maxMessagesPerHour;
  }
  ...
  async voiceGate(userId: string): Promise<"ready" | "consent" | "unavailable" | "rate_limited" | "suspended"> {
    ...
    const [textConsent, voiceConsent] = await Promise.all([this.ai.hasConsent(userId), this.ai.hasProviderConsent(userId, "openai")]);
  ...
  findCardMessage(workspaceId: string, groupId: string): Promise<{ telegramChatId: number; telegramMessageId: number } | null> {
    return this.messages
      .findByPendingGroup(workspaceId, groupId)
      .then((row) => (row?.telegramChatId != null && row.telegramMessageId != null ? { telegramChatId: row.telegramChatId, telegramMessageId: row.telegramMessageId } : null));
  }

  private async withinAiLimits(userId: string, now = new Date()): Promise<boolean> {
    const since = new Date(now.getTime() - 60 * 60_000);
    const [messagesLastHour, callsLastHour] = await Promise.all([this.messages.countUserMessagesSince(userId, since), this.ai.callsLastHour(userId, now)]);
    return aiBurstAllowed({
      messagesLastHour,
      callsLastHour,
      maxMessagesPerHour: this.ai.maxMessagesPerHour,
      maxCallsPerHour: this.ai.maxCallsPerHour,
    });
  }
  ...
  private async processPersistedMessage(input: {
    ...
    try {
      ...
      if (prepared.issues.length) {
        ...
        return {
          kind: "ok",
          text: renderValidationReply(issues, input.language, turn.actions.length),
          appliedCount: 0,
          pendingCount: 0,
          warnings: [],
          ...(topicId ? { topicId } : {}),
      ...
      if (prepared.resolved.length) {
        try {
          ...
          return {
            kind: "ok",
            text: renderValidationReply([issue], input.language, turn.actions.length),
            appliedCount: 0,
            pendingCount: 0,
            warnings: [],
            ...(failedTopicId ? { topicId: failedTopicId } : {}),
          };
        }
      }
      if (liveCard && (acceptsCard || actionResult.pending)) {
        const cancelled = await this.actions.cancel(input.workspaceId, input.userId, liveCard.groupId).catch((error) => {
          logger.error("superseded card could not be cancelled", { groupId: liveCard.groupId, error: safeError(error) });
          return false;
        });
      ...
      await this.messages.setStatus(input.workspaceId, input.inbound.id, "processed").catch((error) => {
        logger.error("message status update failed after successful turn", {
          messageId: input.inbound.id,
          message: safeMessageMetadata(input.inbound.content),
  ...
  private async runModelTurn(input: {
    ...
    try {
      ...
      if (error instanceof AiStructuredOutputError) {
        logger.warn("AI structured output unusable after repair", { error: safeError(error) });
  ...
  private async resolveCardReply(
    ...
    try {
      ...
      logger.warn("typed confirmation failed", { groupId: card.groupId, error: safeError(error) });
  ...
  private logRejectedTurn(messageId: string, turn: AiTurn, issues: readonly ActionIssue[], ctx: TurnContext, timezone: string, now: Date): void {
    logger.warn("AI action rejected", {
      messageId,
      ...
      issues: issues.map((issue) => ({
        ...
        candidateCount: issue.candidates?.length ?? 0,
