...
export interface CreateGoalInput extends GroupScope { title: string; why?: string; targetLocalDate?: string; now: Date }
export interface UpdateGoalInput extends GroupScope {
  goalId: string; expectedVersion: number;
  patch: { title?: string; why?: string; targetLocalDate?: string; status?: "active" | "paused" | "completed" | "cancelled"; reviewEnabled?: boolean };
  now: Date;
}
export interface SaveMemoryInput extends GroupScope { memoryType: MemoryType; content: string; sensitive: boolean; source: LinkSource; sourceMessageId?: string; now: Date }
export interface DeleteMemoryInput extends GroupScope { memoryId: string; expectedVersion: number; now: Date }
export interface UpdateMemoryInput extends GroupScope { memoryId: string; expectedVersion: number; patch: { content?: string; sensitive?: boolean }; now: Date }
export interface LinkTaskToGoalInput extends GroupScope {
  taskId: string; expectedTaskVersion: number; goalId: string; expectedGoalVersion: number; source: LinkSource; confidence: number; now: Date;
}
export interface UnlinkTaskToGoalInput extends GroupScope { taskId: string; expectedTaskVersion: number; goalId: string; expectedGoalVersion: number; now: Date }
export interface GoalPlanInput extends GroupScope {
  goal: { title: string; why?: string; targetLocalDate?: string }; plans: readonly PersistedTaskPlan[]; source: LinkSource; now: Date;
}

export interface CreateGoalStepResult { kind: "create_goal"; goalId: string; title: string }
export interface UpdateGoalStepResult { kind: "update_goal"; goalId: string; title: string }
export interface SaveMemoryStepResult { kind: "save_memory"; memoryId: string; content: string }
export interface UpdateMemoryStepResult { kind: "update_memory"; memoryId: string; content: string }
export interface DeleteMemoryStepResult { kind: "delete_memory"; memoryId: string; content: string }
export interface LinkTaskToGoalStepResult { kind: "link_task_to_goal"; taskId: string; goalId: string; taskTitle: string; goalTitle: string }
export interface UnlinkTaskToGoalStepResult { kind: "unlink_task_to_goal"; taskId: string; goalId: string; taskTitle: string; goalTitle: string }
export interface GoalPlanStepResult { kind: "goal_plan"; goalId: string; goalTitle: string; taskIds: string[]; taskTitles: string[] }
...
export async function createGoalInTx(tx: DbTransaction, input: CreateGoalInput): Promise<InTx<CreateGoalStepResult>> {
  ...
  await tx.insert(actionEvents).values({
    workspaceId: input.workspaceId, groupId: input.groupId, actionType: "create_goal", entityType: "goal",
    entityId: goal.id, postVersion: goal.version, afterState: { title: goal.title },
  });
...
export async function saveMemoryInTx(tx: DbTransaction, input: SaveMemoryInput): Promise<InTx<SaveMemoryStepResult>> {
  ...
  await tx.insert(actionEvents).values({
    workspaceId: input.workspaceId, groupId: input.groupId, actionType: "save_memory", entityType: "memory",
    entityId: memory.id, postVersion: memory.version, afterState: memoryState(memory),
  });
...
export async function goalPlanInTx(tx: DbTransaction, input: GoalPlanInput): Promise<InTx<GoalPlanStepResult>> {
  ...
  await tx.insert(actionEvents).values([
    { workspaceId: input.workspaceId, groupId: input.groupId, actionType: "goal_plan", entityType: "goal", entityId: goal.id, postVersion: goal.version, afterState: { title: goal.title } },
    ...input.plans.map((plan) => ({ workspaceId: input.workspaceId, groupId: input.groupId, actionType: "create_task", entityType: "task", entityId: plan.task.id, postVersion: 1, afterState: { title: plan.task.title, goalId: goal.id } })),
    ...input.plans.map((plan) => ({ workspaceId: input.workspaceId, groupId: input.groupId, actionType: "link_task_to_goal", entityType: "task_goal", entityId: plan.task.id, postVersion: null, afterState: { taskId: plan.task.id, goalId: goal.id, source: input.source, confidence: 100 } })),
  ]);
