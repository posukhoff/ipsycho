...
export function buildSystemPrompt(timezone: string, now: Date, language?: string | null, domainContext?: unknown, correction?: string): string {
  ...
  const lines = [
    ...
    "You may return several create_task actions from one explicit list. Every other action type must be returned one at a time and must not be mixed with create_task actions.",
