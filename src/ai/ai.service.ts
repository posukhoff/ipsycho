  ...
  async respond(input: {
    ...
    try {
      const result = await this.provider.generate({
        model,
        systemPrompt: buildSystemPrompt(input.timezone, input.now ?? new Date(), input.language, redactContextForExternalAi(input.domainContext), input.correction),
...
export function buildSystemPrompt(timezone: string, now: Date, language?: string | null, domainContext?: unknown, correction?: string): string {
  ...
  const lines = [
    ...
    "You may return several create_task actions from one explicit list, or several save_memory actions from one explicit set of durable facts. Every other action type must be returned one at a time and must not be mixed with task or memory batches.",
    ...
    "Use change_series only for a recurring task: pause temporarily, resume a paused series, stop to end future recurrence while preserving the current occurrence, cancel to cancel the series including current unfinished occurrences, or edit to change the future series schedule. For edit provide edit={timezone,recurrenceRule,recurrenceTimezone,missPolicy,plannedStartAt,plannedEndAt,plannedLocalDate,dueAt,dueLocalDate}; keep the same task timeMode and use edit=null for all non-edit operations. Series edit affects future projections only and never rewrites completed history.",
    ...
    "Use ISO-8601 timestamps with an explicit Z or timezone offset for exact times, and make that offset match the IANA timezone field at that instant. Use YYYY-MM-DD for local dates.",
    "When recurrence is explicit, recurrenceRule must be the supported semicolon format without an RRULE: prefix, for example FREQ=DAILY;INTERVAL=1, FREQ=WEEKLY;BYDAY=MO,WE, or FREQ=MONTHLY;BYMONTHDAY=15. A daily point or window task may run several times a day with FREQ=DAILY;BYTIME=09:00,14:30,21:00; provide plannedStartAt as the first occurrence time in the user's timezone.",
