  ...
  async transcribe(input: { workspaceId: string; userId: string; audio: Buffer; durationSeconds: number; language?: string }): Promise<string> {
    ...
    try {
      ...
      await this.repository.recordUsage({
        workspaceId: input.workspaceId, userId: input.userId, provider: "openai", model,
        latencyMs: Date.now() - started, status: "transcription_success",
        ...(pricing ? { pricingRevision: pricing.revision } : {}),
        ...(estimatedCostUsd !== undefined ? { estimatedCostUsd } : {}),
      });
      return response.text.trim();
    } catch (error) {
      await this.repository.recordUsage({ workspaceId: input.workspaceId, userId: input.userId, provider: "openai", model, latencyMs: Date.now() - started, status: "transcription_error" }).catch(() => undefined);
      throw error;
    }
  }
}
