...
test("several created tasks are listed with their own time, reminder and year when it differs", () => {
  const report = renderAppliedReport([
    { kind: "task_created", title: "Вакцинация", timezone: "Europe/Kyiv", importance: "normal", schedule: kyiv("2026-08-23T15:00:00Z"), reminderAt: new Date("2026-08-23T14:30:00Z") },
    { kind: "task_created", title: "Записаться на вакцинацию Морти", timezone: "Europe/Kyiv", importance: "required", schedule: kyiv("2027-08-23T07:00:00Z"), reminderAt: new Date("2027-08-23T07:00:00Z") },
  ], now);
  assert.equal(report, [
    "✅ Создано задач: 2",
...
test("one created task shows schedule, reminder, recurrence and goal link on separate lines", () => {
  const report = renderAppliedReport([
    { kind: "task_created", title: "Зарядка", timezone: "Europe/Kyiv", importance: "normal", recurring: true, schedule: kyiv("2026-08-24T05:00:00Z"), reminderAt: new Date("2026-08-23T18:00:00Z"), goalTitle: "Здоровье" },
  ], now);
...
test("fuzzy and date-only tasks do not invent a clock time", () => {
  const fuzzy = renderAppliedReport([
    { kind: "task_created", title: "Разобрать гараж", timezone: "Europe/Kyiv", schedule: null, fuzzyHorizonText: "на этой неделе", reviewAt: new Date("2026-08-28T06:00:00Z"), reminderAt: null },
  ], now);
  ...
  const deadline = renderAppliedReport([
    { kind: "task_created", title: "Сдать отчёт", timezone: "Europe/Kyiv", schedule: { timezone: "Europe/Kyiv", plannedStartAt: null, plannedEndAt: null, plannedLocalDate: null, dueAt: null, dueLocalDate: "2027-01-15" }, reminderAt: null },
  ], now);
...
test("task update lists every changed field as old → new", () => {
  const changes = taskFieldChanges(
    { title: "Контрольное напоминание о вакцинации", why: "Чтобы напомнить о вакцинации через год.", nextAction: null, context: "Контрольное напоминание ровно через год в 10:00.", importance: "normal", checklist: [] },
    { title: "Записаться на вакцинацию Морти", why: "Чтобы напомнить о вакцинации через год.", nextAction: "Позвонить в клинику", context: null, importance: "required", checklist: [{ text: "a", done: true }, { text: "b", done: false }] },
  );
  assert.equal(renderAppliedReport([{ kind: "task_updated", title: "Записаться на вакцинацию Морти", changes }], now), [
...
test("reschedule shows before → after with the new reminder and reason", () => {
  const report = renderAppliedReport([
    { kind: "task_rescheduled", title: "Созвон", before: kyiv("2026-08-23T15:00:00Z"), after: kyiv("2026-08-24T07:00:00Z"), reminderAt: new Date("2026-08-24T06:30:00Z"), reason: "клиент попросил" },
  ], now);
...
test("reminder, occurrence, series, goal, memory and settings outcomes are all explicit", () => {
  const report = renderAppliedReport([
    ...
    { kind: "occurrence", title: "Отчёт", operation: "record_blocker", details: "нет данных от бухгалтерии" },
  ...
  assert.equal(report, [
    ...
    "✅ Выполнено: «Созвон»",
    "🧱 Блокер записан: «Отчёт» — нет данных от бухгалтерии",
...
test("goal plan lists the goal and each of its tasks", () => {
  const report = renderAppliedReport([{
    kind: "goal_plan", goalTitle: "Английский",
    tasks: [
