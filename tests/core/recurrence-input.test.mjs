...
test("structured recurrence rejects unsupported field combinations", () => {
  assert.throws(() => compileStructuredRecurrence({
    frequency: "weekly", interval: 1, startsOn: "2026-09-07", localTimes: ["09:00", "18:00"],
  ...
  assert.throws(() => compileStructuredRecurrence({
    frequency: "weekly", interval: 1, startsOn: "2026-09-07", localTimes: ["09:00"],
  }, { anchorLocalTime: "14:00" }), /contradicts the schedule start time/);
  assert.throws(() => compileStructuredRecurrence({
...
test("weekly recurrence tolerates the single local time its schedule anchor already carries", () => {
  assert.deepEqual(compileStructuredRecurrence({
    frequency: "weekly", interval: 1, startsOn: "2026-08-23", weekdays: ["SU"], localTimes: ["14:00"],
