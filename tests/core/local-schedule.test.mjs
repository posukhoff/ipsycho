...
test("local schedules compile exact, date-only, deadline and fuzzy modes", () => {
  assert.equal(
    compileStructuredLocalSchedule({ mode: "exact", timezone: "Europe/Kyiv", startDate: "2026-08-25", startTime: "09:30" }).plannedStartAt.toISOString(),
    "2026-08-25T06:30:00.000Z",
  );
  assert.deepEqual(compileStructuredLocalSchedule({ mode: "date", timezone: "Europe/Kyiv", startDate: "2026-08-25" }), {
    timeMode: "window",
    timezone: "Europe/Kyiv",
    plannedLocalDate: "2026-08-25",
  });
  assert.deepEqual(compileStructuredLocalSchedule({ mode: "deadline", timezone: "Europe/Kyiv", dueDate: "2026-08-25" }), {
    timeMode: "deadline",
    timezone: "Europe/Kyiv",
    dueLocalDate: "2026-08-25",
  });
  assert.equal(
    compileStructuredLocalSchedule({
      mode: "fuzzy",
      timezone: "Europe/Kyiv",
...
test("Kyiv DST mapping is deterministic and mode fields cannot conflict", () => {
  assert.equal(
    compileStructuredLocalSchedule({ mode: "exact", timezone: "Europe/Kyiv", startDate: "2026-03-29", startTime: "03:30" }).plannedStartAt.toISOString(),
  ...
  assert.throws(
    () => compileStructuredLocalSchedule({ mode: "window", timezone: "Europe/Kyiv", startDate: "2026-08-25", startTime: "09:30", endTime: "10:00", durationMinutes: 30 }),
    /not both/,
  );
});
