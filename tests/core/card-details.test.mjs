...
test("a reason or context that only talks about the reminder or the schedule is dropped", () => {
  for (const text of ["Чтобы не забыть позвонить", "Напомнить себе о встрече", "Контрольное напоминание ровно через год в 10:00.", "Запланировано на сегодняшний вечер.", "Reminder for the dentist"]) {
...
test("stored cards from production 2026-08-23 lose every line that restates the title or the schedule", () => {
  ...
  assert.deepEqual(show({ title: "Записаться на вакцинацию Морти", why: "Чтобы напомнить о вакцинации через год.", nextAction: "Проверить, нужна ли повторная вакцинация.", context: "Контрольное напоминание ровно через год в 10:00." }),
    { why: null, nextAction: null, context: null });
  assert.deepEqual(show({ title: "Зайти в это приложение", why: "Чтобы не забыть вернуться в приложение через минуту.", nextAction: "Зайти в приложение.", context: "Напоминание через минуту от текущего времени." }),
    { why: null, nextAction: null, context: null });
  ...
  assert.deepEqual(show({ title: "Пройти Soulmate Scan", nextAction: "Открыть Soulmate Scan и пройти его вечером.", context: "Запланировано на сегодняшний вечер." }), { why: null, nextAction: null, context: null });
  ...
  assert.deepEqual(show({ title: "Прийти на вакцинацию собаки сегодня", why: "Сегодня нужно записаться на плановую вакцинацию собаки.", nextAction: "Позвонить в ветклинику и подтвердить запись.", context: "Сегодня вакцинация в 18:00. Есть отдельное контрольное напоминание ровно через год в 10:00." }),
  ...
  assert.deepEqual(show({
    title: "Позвонить в ветклинику по плановой вакцинации для собаки", why: "Чтобы не пропустить плановую прививку и вовремя записать собаку.",
    nextAction: "Позвонить в ветклинику завтра в 10:00.", context: "Звонок для плановой вакцинации собаки; нужно уточнить запись и доступное время.",
    checklist: [{ text: "Найти номер ветклиники", done: false }, { text: "Позвонить", done: false }, { text: "Уточнить запись и время вакцинации", done: false }],
  }), { why: null, nextAction: null, context: null });
});
