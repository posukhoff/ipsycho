...
test("daily recurrence supports several explicit local times", () => {
  ...
  const occurrences = buildRecurringOccurrences(
    {
      ...
      recurrenceRule: "FREQ=DAILY;BYTIME=09:00,14:30,21:00",
      recurrenceTimezone: "Europe/Kyiv",
      missPolicy: "expire",
    },
  ...
  assert.deepEqual(
    occurrences.map((item) => item.recurrenceKey),
    ["2026-08-11T09:00", "2026-08-11T14:30", "2026-08-11T21:00"],
  );
  assert.deepEqual(
    occurrences.map((item) => item.plannedStartAt.toISOString()),
...
test("weekly BYDAY uses local calendar dates", () => {
  ...
  assert.deepEqual(
    occurrences.map((x) => x.recurrenceKey),
    ["2026-08-10", "2026-08-12", "2026-08-17"],
  );
});
