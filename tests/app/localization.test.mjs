...
const task = { id: "t", title: "Call the vet", importance: "required", timezone: "Europe/Kyiv", why: "Annual shot", nextAction: "Find the passport", checklist: [{ text: "Passport", done: true }, { text: "Card", done: false }], goalTitle: "Dog health", nextReminderAt: new Date("2026-08-23T14:30:00Z"), recurrenceRule: "FREQ=WEEKLY;BYDAY=MO,WE" };
const occurrence = { id: "o", status: "open", timezone: "Europe/Kyiv", plannedStartAt: new Date("2026-08-23T15:00:00Z"), plannedEndAt: null, plannedLocalDate: null, dueAt: null, dueLocalDate: null, overdue: true };
...
test("keyboards, labels and the applied report follow the locale", () => {
  ...
  assert.ok(labels(settingsKeyboard("en", { morningDigestEnabled: true, eveningDigestEnabled: false, weeklyReviewEnabled: true, quietHoursEnabled: false })).includes("☀️ Morning: on"));
  ...
  const report = renderAppliedReport([
    { kind: "task_updated", title: "Report", changes: [{ field: "importance", before: "normal", after: "critical" }, { field: "checklist", before: null, after: "checklist:2:1" }] },
  ...
  assert.match(renderAppliedReport([{ kind: "task_updated", title: "Отчёт", changes: [{ field: "importance", before: "normal", after: "critical" }] }], now, "ru"), /«обычная» → «критическая»/);
});
...
test("a confirmation card names the task or goal it is about, in the user's language", () => {
  ...
  assert.equal(describeAction({ ...base, type: "goal", op: "link", goalId: "33333333-3333-4333-8333-333333333333", goalVersion: 1, taskId: target.taskId, taskVersion: 1, title: null, why: null, targetDate: null, status: null, reviewEnabled: null }, "ru", names), "Связать «Созвон с дизайнером» с целью «Запуск»");
  assert.equal(describeAction({ ...base, type: "settings", operation: "quiet_hours", timezone: "Europe/Kyiv", applyTimezoneTo: null, language: null, digestKind: null, enabled: true, time: null, weekday: null, weekdayStart: "23:00", weekdayEnd: "08:00", weekendStart: "23:00", weekendEnd: "10:00", snoozeUntilDate: null, snoozeUntilTime: null, eventOffsets: null, plannedTaskOffsetMinutes: null, criticalPostDueMinutes: null, seenNormalMinutes: null, seenRequiredMinutes: null, seenCriticalMinutes: null, expectedVersion: 1 }, "ru"), "Изменить настройки: тихие часы → on, 23:00–08:00, 23:00–10:00");
